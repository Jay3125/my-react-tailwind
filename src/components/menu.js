...
const Menu = ({ children }) => {
  ...
  return (
    <>
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} fixed top-0 right-0 w-full h-full bg-black opacity-50 z-40`} onClick={toggleMenu}></div>
      ...
      <button className="py-2 px-2 bg-sky-500" onClick={toggleMenu}>Open Menu</button>
      <button className="py-2 px-2 bg-sky-500" onClick={toggleMenu}>Open Menu</button>
